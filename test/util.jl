...
@testset "$ST strings" begin
    @testset "padding (lpad and rpad)" begin
        ...
        for (p, lr, rr) in ((2, "foo", "foo"), (3, "foo", "foo"), (4, "1foo", "foo1"),
            ...
            @test lpad(str1, p, pad1) == lr
            @test rpad(str1, p, pad1) == rr
        end
        for (p, lr, rr) in ((2, "αβ", "αβ"), (3, "¹αβ", "αβ¹"), (4, "¹₂αβ", "αβ¹₂"),
                            (5, "¹₂³αβ", "αβ¹₂³"), (6, "¹₂³¹αβ", "αβ¹₂³¹"),
                            (7, "¹₂³¹₂αβ", "αβ¹₂³¹₂"), (8, "¹₂³¹₂³αβ", "αβ¹₂³¹₂³"),
                            (9, "¹₂³¹₂³¹αβ", "αβ¹₂³¹₂³¹"))
            @test lpad(str2, p, pad2) == lr
            @test rpad(str2, p, pad2) == rr
        end
    end
    ...
    @test split(ST(","), comma) == ["",""]
    @test split(ST(",,"), comma) == ["","",""]
    @test split(ST(""), comma  ; keep=false) == []
    @test split(ST(","), comma ; keep=false) == []
    @test split(ST(",,"), comma; keep=false) == []
    ...
    @test rsplit(ST(""), comma  ; keep=false) == []
    @test rsplit(ST(","), comma ; keep=false) == []
    @test rsplit(ST(",,"), comma; keep=false) == []
